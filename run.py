...
def main(node_type, args):
    if node_type in {"chord", "scrapchord", "sc"}:
        port = int(args[0])
        m = int(args[1])
        join_ip = args[2] if len(args) > 2 else ""
        visible = int(args[3]) if len(args) > 3 else 8
        forever = False if len(args) > 4 and args[4] == 'f' else True
        ...
        visible = int(args[2]) if len(args) > 2 else 15
        forever = False if len(args) > 3 and args[3] == 'f' else True
        s.run(visible=visible, forever=forever)
    
    elif node_type in {"client", "cli", "c"}:
        port = int(args[0])
        m = int(args[1])
        addr = args[2] if len(args) > 2 else ""
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("DEBUGING")
        node_type = "sc"
        args = ["7070", "5", "aaa", "32", "f"]#, "127.0.1.1:7050"]
    main(node_type, args)