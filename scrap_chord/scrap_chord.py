    ...
    def run(self, addr: str = "", mss_per_thread: int = 30):
        ...
        if addr != "":
            ...
            if len(net_nodes) > 0:
                for node_addr in net_nodes:
                    self.add_node(
                        (
                            get_id(address_to_string(node_addr)) % (2 ** self.bits),
                            (node_addr),
                        )
                    )
        ...
        while self.online:
            ...
            if inp[0] == "ky":
                ...
                print("Replica:")
                print(self.pred_replica.get_all())
            if inp[0] == "of":
                self.online = False
    ...
    def communicate_with_client(self):
        ...
        while self.online:
            ...
            if self.chord_scrap_pipe[0] in socks:
                ...
                self.logger.debug(
                    f"CliCom: ({forwards_count})Forwarding url result to client"
                )
                forwards_count += 1
    ...
    def communicate_with_scraper(self, messages_per_thread):
        ...
        while self.online:
            ...
            if self.chord_scrap_pipe[1] in socks:
                ...
                if self.storage.has_key(url_id):  # url in self.cache:
                    url_id, (html, url_list) = self.storage.get_key(url_id)
                ...
                url_id = self.url_id(url)
                if self.storage.has_key(url_id):
                    continue
                # remove from pending
                try:
                ...
                self.storage.insert_pair(url_id, (html, url_list))
                # forward to comm client
                self.chord_scrap_pipe[1].send_pyobj((url, html, url_list))

            elif (
                not connected_to_any_scraper
